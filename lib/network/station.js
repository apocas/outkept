...
Station.prototype.start = function () {
  ...
  io.sockets.on('connection', function (socket) {
    ...
    socket.on('authenticate', function (credentials) {
      ...
      md5sum.update('' + credentials.password + ':' + credentials.username);
      this.sessionid = md5sum.digest('hex');
      this.emit('authentication', {'result': this.authenticated, 'sessionid': this.sessionid});
    });
