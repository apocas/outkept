...
Outkept.prototype.createServer = function (hostname, sshport) {
  ...
  s.on('alarmed', function (sensor) {
    ...
    outils.sendEvent('trigger', 'alarmed', sensor.conf.name, sensor.value, s);
  });

  s.on('warned', function (sensor) {
    console.log(this.hostname + ' WARNING!');
    outils.sendEvent('trigger', 'warned', sensor.conf.name, sensor.value, s);
  });
  ...
  s.on('fired', function (sensor) {
    ...
    outils.sendEvent('reactive', 'fired', sensor.conf.name, sensor.value, s);
  });
