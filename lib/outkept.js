...
var Outkept  = function (passphrase) {
  ...
  //this.eraseKey();
  this.redis.hset(outils.getHashDate(), 'sensors', 0);
  ...
  this.feeds.on('alert', function (feed, data) {
    self.redis.sismember(['feeds', data], function (err, member) {
      if(member != 1) {
        ...
        self.redis.hincrby(outils.getHashDate(), 'feeds', 1);
      }
    });
  });
...
Outkept.prototype.countSensors = function () {
  ...
  this.redis.hset(outils.getHashDate(), 'sensors', stotal);
};
...
Outkept.prototype.addServer = function (hostname, sshport) {
  ...
  s.on('ready', function () {
    ...
    if (server === undefined) {
      ...
      this.loadSensors();
      self.redis.hset(outils.getHashDate(), 'servers', self.servers.length);
      self.countSensors();
    } else {
